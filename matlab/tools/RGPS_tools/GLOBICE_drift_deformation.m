function [out]=GLOBICE_drift_deformation(startdate_str,enddate_str,compute_deformation,limit_domain)
% Uses the GLOBICE image pairs dataset downloaded from the Globice website and calculates sea ice
% deformations from the image pairs.
%
% example: GLOBICE_drift_deformation('2008_01_01','2008_02_01',1,[])
...
if(compute_deformation)
    ...
    min_size_stream=200;
    nb_sampling=2;          % typical value = 2 (to get the same initial resolution as the RGPS and to decrease the noise)
...
if(compute_deformation)
    ...
    if(filter_noise)
        outfile = ['GLOBICE_deformation_min_' num2str(min_size_stream) '_eps_00' num2str(min_def*100) '_level_' num2str(max_level) '.mat'];
    else
        outfile = ['GLOBICE_deformation_min_' num2str(min_size_stream) '_no_filter.mat'];
    end
end
...
if(startyear==endyear)
    for month=str2num(startmonth):str2num(endmonth)
        files=[dir([indir 'GLOBICE_ice_drift/EULIMGPAIR/', startyear,'/EULIMGPAIRV_KST_' , startyear , sprintf('%02d',month), '*']); files];
    end
else
    ...
    for month=1:str2num(endmonth)
        files=[dir([indir 'GLOBICE_ice_drift/EULIMGPAIR/',   endyear,'/EULIMGPAIRV_KST_' ,   endyear , sprintf('%02d',month), '*']); files];
    end
end
...
if(last_image_pair == 0)
    [indir 'GLOBICE_ice_drift/EULIMGPAIR/', endyear,'/EULIMGPAIRV_KST_*']
...
for i=first_image_pair:last_image_pair
    ...
    date_before=datenum(filename(17:31),'yyyymmdd_HHMMSS');
    date=       datenum(filename(32:46),'yyyymmdd_HHMMSS');
    pair_image=filename(48:51);
    ...
    if(date_before>dnum_start && date<dnum_end && date_before<date)
        ...
        n2 = FINFO.Dimensions.Length;
        
        if(compute_deformation)
            if n2>min_size_stream*nb_sampling^2
                
                %x=ncread(filename,'Location_B_X')*1000; % m
                %y=ncread(filename,'Location_B_Y')*1000; % m
                x=ncread(filename,'Location_A_X')*1000; % m
                y=ncread(filename,'Location_A_Y')*1000; % m
                
                trux2=ncread(filename,'Disp_X')*1000/deltat; % m/day
                trvy2=ncread(filename,'Disp_Y')*1000/deltat; % m/day
                
                % Subsampling
                subsampling_resolution=5000*nb_sampling;
                selected=(~rem(x,subsampling_resolution).*~rem(y,subsampling_resolution));%.*(qflag_1<4));
                
                x=x(selected==1);
                y=y(selected==1);
                trux2=trux2(selected==1);
                trvy2=trvy2(selected==1);

function [out]=search_SAR_drif(startdate_str,enddate_str,compute_deformation)
% Phil: Used to find sar drift pairs from Dennis in Barents-Kara which
% might have data in tha Kara gate. 
% Only works for 1 month at a time
%Uses the lagrangian SAR sea ice motion
% dataset produced by Denis Demchev (NIERSC) for Barents and Kara seas
%
% example: search_SAR_drif('2008_01_01','2008_01_31',0)
...
showplot = 0;    % show some extra triangulation plots (slows down program)
...
if(startyear==endyear)
    for month=str2num(startmonth):str2num(endmonth)
        files=[dir([startyear , sprintf('%02d',month), '*']), files];
    end
else
    for month=str2num(startmonth):12
        files=[dir([startyear , sprintf('%02d',month), '*']), files];
    ...
    for month=1:str2num(endmonth)
        files=[dir( [endyear , sprintf('%02d',month), '*']), files];
    end
end
...
for i=first_image_pair:last_image_pair
    ...
    if(date_before>dnum_start && date<dnum_end && date_before<date)
        ...
        if(compute_deformation)
            ...
            %Selecting only if data in the karagate
%            if (~isempty(find(out.xy(jstart:jend,1)>2000)) & ~isempty(find(out.xy(jstart:jend,1)<2300))  & ~isempty(find(out.xy(jstart:jend,2)<650)) & ~isempty(find(out.xy(jstart:jend,2)>350)))  
%                   load arctic_coasts.mat;
            if (~isempty(find(out.xy(jstart:jend,1)>700)) & ~isempty(find(out.xy(jstart:jend,1)<1300))  & ~isempty(find(out.xy(jstart:jend,2)<700)) & ~isempty(find(out.xy(jstart:jend,2)>1300)))  
                   load arctic_coasts.mat;
...
%                    colormap(jet(256));
                   figure
                   scatter(out.xy(jstart:jend,1),out.xy(jstart:jend,2),4,(out.uv(jstart:jend,1).^2+out.uv(jstart:jend,2).^2).^(0.5))
                   colormap(jet(256));
                   caxis([0 10])
                   colorbar
                   hold on

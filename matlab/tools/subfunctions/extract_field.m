function [field_tmp, field_plotted]=extract_field(field,data_out,dirname,step,simul_in,quiet,no_error)
...
%---------------------------
  ...
  elseif strcmp(field,'Skin_temperature')
     fld = 'SST';
     [sst]=get_and_check(fld,data_out,dirname,step,quiet,no_error);
     ...
     [conc_thin]=get_and_check(fld,data_out,dirname,step,true,true);
     if isempty(tsurf_thin)
         tsurf_thin = 0;
     end
     if isempty(conc_thin)
         conc_thin  = 0;
     end
     
     fld = 'Concentration';
     [conc]=get_and_check(fld,data_out,dirname,step,quiet,no_error);
...
%      field_plotted = 'Total atmospheric heat flux';
  ...
  elseif strcmp(field,'Total_thickness')
     ...
     fld = 'Thin_ice';
     [field_tmp2]=get_and_check(fld,data_out,dirname,step,true,true);
     if isempty(field_tmp2)
     	  field_tmp2=0.;
     end
  ...
  elseif strcmp(field,'New_critical_external_stress')
    ...
    [field_tmp]=get_and_check(field,data_out,dirname,step,quiet,no_error);
    field_plotted=field;
  end
end

function resplot(field,step,dir1,dir2,plot_edge)
%% CALL: resplot(field,step,dir)
if nargin==1, step='init'; end
if nargin<=2, dir1=''; end
if nargin<=3, dir2=''; end
if nargin<=4, plot_edge=false; end
...
[mesh_out1,data_out1] = neXtSIM_bin_revert(dir1,[], step);
[mesh_out2,data_out2] = neXtSIM_bin_revert(dir2,[], step);
...
if(~isempty(field))
    field_tmp1=data_out1.(field);
    field_tmp2=data_out2.(field);
else
    ...
    var_mc1=field_tmp1(mesh_out.Elements);
    c1{1}=reshape(var_mc1,[3,Ne]);
    var_mc1=field_tmp1(mesh_out.Elements+Nn);
    c1{2}=reshape(var_mc1,[3,Ne]);
    
    var_mc2=field_tmp2(mesh_out.Elements);
    c2{1}=reshape(var_mc2,[3,Ne]);
    var_mc2=field_tmp2(mesh_out.Elements+Nn);
    c2{2}=reshape(var_mc2,[3,Ne]);
    
    cos_m_diff_angle=cos(-130/180*pi);
    sin_m_diff_angle=sin(-130/180*pi);
    
    c2_1_turned= cos_m_diff_angle*c2{1}+sin_m_diff_angle*c2{2};
    c2_2_turned=-sin_m_diff_angle*c2{1}+cos_m_diff_angle*c2{2};
    
    c{1}=c1{1}-c2_1_turned;
    c{2}=c1{2}-c2_2_turned;

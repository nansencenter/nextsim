...
# Run from start
$execfile --exporter.precision=double --exporter.path=$tmpdir_start --restart.start_from_restart=false --simul.duration=$duration --restart.write_restart=true --restart.output_time_step=$restart_time_step --output.output_per_day=$output_per_day --config-files=$cfgfile 2>&1 | tee $tmpdir_start/output.log || exit 5

# Copy the restart files to ../restart so they can be read by the model
cp $tmpdir_start/restart/* ../restart/

# Run with a restart
$execfile --exporter.precision=double --exporter.path=$tmpdir_restart --restart.start_from_restart=true --restart.step_nb=$step_nb --simul.duration=$duration --restart.write_restart=false --output.output_per_day=$output_per_day --config-files=$cfgfile 2>&1 | tee $tmpdir_restart/output.log || exit 6

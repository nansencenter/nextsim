...
GridOutput::parallel_initNetCDF(std::string filename)
{
    ...
    //nc_create(filename.c_str(), NC_NETCDF4, &ncid);
...
//    nc_enddef(ncid);
    ...
    int row_offset = 0;
    int col_offset = 0;
/*    boost::mpi::all_gather(M_comm, M_nrows, local_rows);
    for (int i = 0; i < M_comm.rank(); i++) row_offset += local_rows[i];
    boost::mpi::all_gather(M_comm, M_ncols, local_cols);
    for (int i = 0; i < M_comm.rank(); i++) col_offset += local_cols[i];*/
...
//    boost::mpi::broadcast(M_comm, &M_use_lsm, 1, 0);

    // Apply mask - if needed
    if (M_use_lsm)
    {
//        boost::mpi::broadcast(M_comm, &M_lsm[0], M_grid_size, 0);
        this->applyLSM();
    }
...
void
GridOutput::data_exchange(std::vector<std::vector<std::vector<double>>>& nodal_send, std::vector<std::vector<std::vector<double>>>& nodal_recv,

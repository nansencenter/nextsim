...
# Run with one thread
OMP_NUM_THREADS=$num_threads1 $execfile --exporter.precision=double --output.exporter_path=$tmpdir1 --simul.duration=$duration --output.output_per_day=$output_per_day --config-files=$cfgfile 2>&1 | tee $tmpdir1/output.log  || exit 5

# Run with 8 threads
OMP_NUM_THREADS=$num_threads2 $execfile --exporter.precision=double --output.exporter_path=$tmpdir2 --simul.duration=$duration --output.output_per_day=$output_per_day --config-files=$cfgfile 2>&1 | tee $tmpdir2/output.log  || exit 6

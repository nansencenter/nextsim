...
FiniteElement::regrid(bool step)
{
    ...
    if ( (vm["nextwim.coupling-option"].template as<std::string>() == "breaking_on_mesh"))
        // if doing breaking on mesh
        // - then we always need to pass in regridded Nfloes
        M_interp_fsd = M_use_wim;
    else
        // only regrid if not at a coupling time step
        // - else Nfloes will just be overwritten with wimToNextsim()
        M_interp_fsd = (M_use_wim && (!M_run_wim));
#endif
...
FiniteElement::update()
{
    ...
    M_interp_fsd = M_use_wim;
    if (M_interp_fsd)
        std::cout<<"IN UPDATE: "<< "interpolate FSD\n";
    else
        std::cout<<"IN UPDATE: "<< "do not interpolate FSD\n";
#endif
...
FiniteElement::updateVelocity()
{
    ...
    M_solution->container(&M_VT[0]); // this version is more rapid as it directly accesses the memory and does not create a new vector 0.16 ms instead of 0.23 ms for container()

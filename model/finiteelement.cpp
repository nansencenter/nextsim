...
void
FiniteElement::setReloadDatasets()
{
...
FiniteElement::initOptAndParam()
{
    ...
    LOG(DEBUG) << "mooring_time_factor " << mooring_time_factor << "\n";
    ...
    LOG(DEBUG) << "exponent_relaxation_sigma " << exponent_relaxation_sigma << "\n";
    ...
    LOG(DEBUG) << "lin_drag_coef_water  " << lin_drag_coef_water  << "\n";
    ...
    LOG(DEBUG) << "deltaT_relaxation_damage " << deltaT_relaxation_damage << "\n";
    ...
    LOG(DEBUG) << "M_ocean_albedo " << M_ocean_albedo << "\n";
...
FiniteElement::flip(FEMeshType const& mesh, std::vector<double> const& um, double factor) const
{
    ...
    {
        area[cpt] = mesh.jacobian(*it,um,factor);
        ++cpt;
    }
...
FiniteElement::shapeCoeff(element_type const& element) const
{
    ...
    double const jac = M_mesh.jacobian(element, M_UM, 1.);
    for (int k=0; k<3; ++k)
    {
        int const kp1 = (k+1)%3;
        int const kp2 = (k+2)%3;
...
FiniteElement::advect(std::vector<double> const& interp_elt_in, std::vector<double>& interp_elt_out)
{
    ...
    for (int cpt=0; cpt < M_num_elements; ++cpt)
    {
        ...
        for(int i=0;i<3;i++)
        {
            ...
            {
                surface = M_mesh.measure(M_elements[cpt],UM_P);
            ...
            else
            {
                ...
                {
                    surface = M_mesh.measure(M_elements[neighbour_int-1], UM_P);
                    outer_fluxes_area[i] = -std::min(surface/dtime_step/3.,-outer_fluxes_area[i]);
        ...
        surface = M_mesh.measure(M_elements[cpt], UM_P);
        surface_new = M_mesh.measure(M_elements[cpt],M_UM);
...
FiniteElement::advectRoot(std::vector<double> const& interp_elt_in, std::vector<double>& interp_elt_out)
{
    ...
    if (M_rank == 0)
    {
        ...
        for (int cpt=0; cpt < num_elements; ++cpt)
        {
            ...
            surface = M_mesh_root.measure(elements_root[cpt],UM_P_root);
            surface_new = M_mesh_root.measure(elements_root[cpt],M_UM_root);
            ...
            for(int i=0;i<3;i++)
            {
                ...
                else
                {
                    ...
                    if (!std::isnan(neighbour_double) && neighbour_int>0)
                    {
                        double surface_local = M_mesh_root.measure(elements_root[neighbour_int-1], UM_P_root);
    ...
    for (int cpt=0; cpt < M_num_elements; ++cpt)
    {
        M_surface[cpt] = M_mesh.measure(M_elements[cpt],M_UM);
    }
#endif
}//advectRoot
#endif
...
FiniteElement::interpFields(std::vector<int> const& rmap_nodes, std::vector<int> sizes_nodes)
{
    ...
    if (M_rank == 0)
    {
        ...
        {
            surface_previous[cpt] = M_mesh_previous_root.measure(*it);
            ++cpt;
        }
        ...
        {
            surface_root[cpt] = M_mesh_root.measure(*it);
            ++cpt;
        }
...
FiniteElement::regrid(bool step)
{
    ...
    this->assignVariables(M_num_elements, M_num_nodes);
    this->setReloadDatasets();
}//regrid
...
FiniteElement::init()
{
    ...
    else
    {
        ...
        this->initVariables(M_num_elements, M_num_nodes);
        this->setReloadDatasets();
    }
...
FiniteElement::readRestart(std::string const& name_str)
{
    ...
    M_surface = M_mesh.surface(M_UM, 1.);
}//readRestart
        ...
        bool const& export_fields, bool const& apply_displacement)
{
    ...
    if(export_fields)
    {
        M_surface_root = M_mesh_root.surface();

...
namespace Nextsim
{
...

    {
        ...
        desc.add_options()
            ...
            // - Internal stresses
            ("dynamics.C_perturb", po::value<double>()->default_value( 0.0 ), "")     // Fraction of C_fix that will be added to C_fix as some alea on the cohesion as C_fix ± C_perturb
            ...
            ("statevector.restart_path", po::value<std::string>()->default_value( "" ), "where to find restart files mem???.nc.analysis")  // must be subfolder name of nextsimDataDir, it is used in dataset.cpp enkf_analysis_elements

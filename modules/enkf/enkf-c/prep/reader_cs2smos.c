...
void reader_cs2smos_standard(char* fname, int fid, obsmeta* meta, grid* g, observations* obs)
{
    ...
    if (strcmp(meta->type,"SIT") == 0) {  // type in obstypes.prm
    ...
    for (int it= 0; it< (int) tlen; ++it) {
        for (int i = 0; i < (int) ylen; ++i) {
            for (int j = 0; j < (int) xlen; ++j) {
                ...
                o->fid = fid;
                o->batch = 0;
                if (strcmp(meta->type,"SIT") == 0) {
                    o->value = (float) (sit[it][i][j]*sit_scale_factor); 
                    o->estd  = (float) (error_std[it][i][j]*estd_scale_factor); //data error from interpolation
                    // o->estd   = (double) fmin(0.5, 0.1 + 0.15*sit[it][i][j]* sit_scale_factor); // Xie et al. (2017)
                // This correction accounts for other errors, including the sensor errors and the model related representation error, diagnosed observation errors, the bias (inconsistency) between the CryoSat2 and SMOS observations. Xie()
                    if (o->value<3 && o->value>0) 
                        o->estd += fmax(0.02,0.1*exp(-o->value*1.5)); // Offset 0.1-0.02
                    else
                        o->estd += fmin(0.2,0.02*exp(1.8*(o->value-3))); // Offset 0.02-0.2

...
! log 15-2-2011
! - generate grid from ASR and ECMWF forcing files. Done
...
! ----------------------------------------------------------------------------
   ...
   real, allocatable, dimension(:,:)  :: synuwind, synvwind,synsnowfall,syndwlongw !,synwndspd
...
contains
   ...
   subroutine init_rand_update(synforc,randfld,previous_perturbation_exist)
      use mod_pseudo
      implicit none
      integer:: previous_perturbation_exist
      real*8, dimension(idm*jdm,4) :: synforc,randfld ! dimensional and nondimensional forcing_fields
      if(.not.randf) then
      ...
      if (previous_perturbation_exist==1) then            
         if (debug) print*,  'load nondimensional-previous perturbations in externaldata.cpp'
         call load_randfld(randfld)  
      else
         if (debug) print*, 'generating initial random field...'
         call ranfields(ran,rh)
         call rand_update(randfld, synforc) ! update randfld, synforc, and ran1, ran (final nondimensional fields, ran1 is temporary variable)
      end if
      call rand_update(randfld, synforc) 
      call save_randfld_synforc(randfld, synforc) ! save ran to randfld and syn* to synforc, returning to c++
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  instruction of setting pseudo2D.nml
      ! &setup             !namelist
      ! iopath = '.'       ! file path
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real    :: scorr, scorr_dx, tcorr, vsnowfall, vdwlongw !, vairtmp, vprecip, vrelhum
      character(80) :: nmlfile, cwd

      namelist /setup/ iopath, debug
      namelist /pseudo2D/ randf, seed, vslp, vwndspd, vsnowfall, vdwlongw, & 
...
! --- the random forcing is added to the forcing fields.
   subroutine rand_update(randfld, synforc)
      implicit none
      ...
      real*8  :: randfld(idm*jdm,4), synforc(idm*jdm,4)
...
!------------------------------------------
   subroutine load_randfld(randfld) 
      integer :: ix,jy,id
      real*8  :: randfld(idm*jdm,4)
      do jy=1,jdm
      do ix=1,idm            
         ! the order of x,y and id is related to the order of loading wind data in loadDataset() in externaldata.cpp
         id = (jy-1)*idm + ix  
         ran%slp(ix,jy)     = randfld(id,1)
         ran%wndspd(ix,jy)  = randfld(id,2)
         ran%snowfall(ix,jy)= randfld(id,3)
         ran%dwlongw(ix,jy) = randfld(id,4)
      end do 
      end do 
   end subroutine
...
!------------------------------------------
   subroutine save_randfld_synforc(randfld, synforc)
      integer :: ix,jy,id
      real*8  :: randfld(idm*jdm,4), synforc(idm*jdm,4) ! same dimension size as defined in externaldata.cpp
      
      do jy=1,jdm      
      do ix=1,idm
         id = (jy-1)*idm + ix
         randfld(id,:) = (/ ran%slp(ix,jy),ran%wndspd(ix,jy), ran%snowfall(ix,jy),ran%dwlongw(ix,jy) /)
         synforc(id,:) = (/ synuwind(ix,jy), synvwind(ix,jy), synsnowfall(ix,jy), syndwlongw(ix,jy)  /)
      end do 
      end do 
   end subroutine

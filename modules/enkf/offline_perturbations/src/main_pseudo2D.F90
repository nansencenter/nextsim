program main_pseudo2D
  ...
  FILE_NAME='randfld_for_restart.dat'    ! randfld=(/ ran%slp,ran%wndspd,ran%snowfall,ran%dwlongw,ran%sss,ran%sst /)
  ...
  do i_step = 0,10*4 ! create a series of perturbations for one member
      ...
      call init_rand_update(synforc, randfld, 0) 
      ...
      ! Define the variable. The order is consisteng with the order in fun. save_randfld_synforc()
      ...
      ! ! save synforc and randfld in .dat
      ! write(fileID,'(I4)') i_step
      ! FILE_NAME='synforc_randfld'//trim(adjustl(fileID))//'.dat'
      ! open(13,file=trim(FILE_NAME))
      ! do id=1,xy_full
      !    write(13,'(12f8.4)') synforc(id, :), randfld(id, :)
      ! end do
      ! close(13)
   enddo
...
! ecmwf forecast 688*556
! topaz forecast 609*881

...
contains
   ...
   subroutine init_rand_update(synforc, randfld, previous_perturbation_exist)
      ...
      real*8, dimension(idm*jdm,6) :: synforc, randfld ! dimensional and nondimensional forcing_fields
      if (.not. randf) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      namelist /pseudo2D/ randf, seed, vslp, vwndspd, vsnowfall, vdwlongw, vsss, vsst, &
...
!------------------------------------------
   subroutine load_randfld(randfld)
      ...
      do ix = 1, idm
         ...
         ran%sss(ix,jy)     = randfld(id,5)
         ran%sst(ix,jy)     = randfld(id,6)
      end do
      end do
   end subroutine
...
!------------------------------------------
   subroutine save_randfld_synforc(randfld, synforc)
      integer :: ix, jy, id
      real*8  :: randfld(idm*jdm, 6), synforc(idm*jdm, 6) ! same dimension size as defined in externaldata.cpp

      do jy = 1, jdm
      do ix = 1, idm
         id = (jy - 1)*idm + ix
         randfld(id,:)=(/ ran%slp(ix,jy),ran%wndspd(ix,jy),ran%snowfall(ix,jy),ran%dwlongw(ix,jy),ran%sss(ix,jy),ran%sst(ix,jy) /)
         synforc(id,:)=(/ synuwind(ix,jy), synvwind(ix,jy),synsnowfall(ix,jy), syndwlongw(ix,jy), synsss(ix,jy), synsst(ix,jy)  /)
      end do
      end do
   end subroutine

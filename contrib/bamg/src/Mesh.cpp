...
namespace bamg {
	...
	void Mesh::ReadMesh(int* index,double* x,double* y,int nods,int nels){/*{{{*/
		...
		vertices=new BamgVertex[nbv];
		orderedvertices=new BamgVertex* [nbv];
		for (i=0;i<nbv;i++){
			vertices[i].r.x=x[i];
			vertices[i].r.y=y[i];
	...
	void Mesh::WriteMesh(BamgMesh* bamgmesh,BamgOpts* bamgopts){/*{{{*/
		...
		//Memory Allocation
		head_1 = new int[nbv];
		next_1 = new int[3*nbt];
		connectivitysize_1= new int[nbv];
		...
		if(nbv){
			bamgmesh->Vertices=new double[3*nbv];
			bamgmesh->PreviousNumbering=new double[nbv];
			for (i=0;i<nbv;i++){
		...
		if (nbe){
			bamgmesh->Edges=new double[3*nbe];
			for (i=0;i<nbe;i++){
		...
		bamgmesh->IssmEdges=new double[4*edge4->nb()];
		...
		bamgmesh->IssmSegments=new double[4*NumIssmSegments];
		num=0;
		for (i=0;i<nbe;i++){
			if(edges[i].GeomEdgeHook){
				//build segment
		...
		if (k){
			bamgmesh->Triangles=new double[4*k];
			for (i=0;i<nbt;i++){
				Triangle &t=triangles[i];
		...
		if (nbsubdomains){
			bamgmesh->SubDomains=new double[4*nbsubdomains];
			for (i=0;i<nbsubdomains;i++){
		...
		if (Gh.nbsubdomains){
			bamgmesh->SubDomainsFromGeom=new double[4*Gh.nbsubdomains];
		...
		if (NbVerticesOnGeomVertex){
			bamgmesh->VerticesOnGeomVertex=new double[2*NbVerticesOnGeomVertex];
			for (i=0;i<NbVerticesOnGeomVertex;i++){
		...
		if (NbVerticesOnGeomEdge){
			bamgmesh->VerticesOnGeomEdge=new double[3*NbVerticesOnGeomEdge];
			for (i=0;i<NbVerticesOnGeomEdge;i++){
		...
		if (k){
			bamgmesh->EdgesOnGeomEdge=new double[2*(int)k];
			int count=0;
			for (i=0;i<nbe;i++){
				if (edges[i].GeomEdgeHook){
		...
		head_2=new int[nbv];
		next_2=new int[2*i1];
		connectivitysize_2=new int[nbv];
		//Initialization
		...
		bamgmesh->NodalConnectivity=new double[connectivitymax_2*nbv];
	...
	void Mesh::WriteMetric(BamgOpts* bamgopts) {/*{{{*/
		int i;
		delete bamgopts->metric;
		bamgopts->metric=new double[3*nbv];
		for (i=0;i<nbv;i++){
	...
	void Mesh::CreateSingleVertexToTriangleConnectivity(){/*{{{*/
	...
	void Mesh::UnMarkUnSwapTriangle(){/*{{{*/
		for (int i=0;i<nbt;i++)
		 for(int j=0;j<3;j++)
		  triangles[i].SetUnMarkUnSwap(j);
	}/*}}}*/
	void Mesh::SetVertexFieldOn(){/*{{{*/

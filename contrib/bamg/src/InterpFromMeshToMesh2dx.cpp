...
int InterpFromMeshToMesh2dx(double** pdata_interp,
      ...
      bool isdefault, double defaultvalue /*Options* options*/)
{
...
#pragma omp parallel for num_threads(max_threads) private(thread_id)
    ...
    for (int i=0; i < N_interp; ++i)
    {
        ...
        if (M_data==nods_data)
        {
            for (j=0;j<N_data;j++)
            {
                data_interp[i*N_data+j]=areacoord[0]*data[N_data*i0+j]
                    +areacoord[1]*data[N_data*i1+j]
                    +areacoord[2]*data[N_data*i2+j];
            }
        }
        else{
         for (j=0;j<N_data;j++)

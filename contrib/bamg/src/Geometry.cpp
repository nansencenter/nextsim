...
namespace bamg {
	...
	void Geometry::WriteGeometry(BamgGeom* bamggeom, BamgOpts* bamgopts){/*{{{*/
		...
		if (nbv){
			bamggeom->Vertices=new double[3*nbv];
			for (i=0;i<nbv;i++){
		...
		if (nbe){
			bamggeom->Edges=new double[3*nbe];
			for (i=0;i<nbe;i++){
		...
		if (nbreq){
			bamggeom->RequiredEdges=new double[1*nbreq];
			count=0;
			for (i=0;i<nbe;i++){
				if (edges[i].Required()){
		...
		if (nbreqv){
			bamggeom->RequiredVertices=new double[1*nbreqv];
			count=0;
			for (i=0;i<nbv;i++){
		...
		if (nbsubdomains){
			bamggeom->SubDomains=new double[4*nbsubdomains];
			for (i=0;i<nbsubdomains;i++){
		...
		if (nbtan){
			bamggeom->TangentAtEdges=new double[4*nbtan];
			for (i=0;i<nbe;i++){
